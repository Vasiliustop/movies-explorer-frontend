...
export default function SearchForm({
  ...
  function getStatusCheckBoxAndInput() {
    ...
    if (location.pathname === "/saved-movies") {
      localStorage.setItem(
        ...
        JSON.stringify({ checkBox: false, request: "", movies: cards })
      );
  ...
  function soldCheckBox(e) {
    if (location.pathname === "/movies") {
      ...
      localStorage.setItem(
        "checkBox",
        JSON.stringify({ checkBox: statusCheckBox })
      );
    ...
    if (location.pathname === "/saved-movies") {
      ...
      if (e.target.checked) {
        const searchResult = asd(savedMoviess, e.target.checked);
    ...
    <form className="search-form" onSubmit={handleSumbit}>
      <fieldset className="form__container">
        <input
          ...
          name="searchinput"
          onChange={(e) => {
            handlerSearchForm(e, location.pathname);
          }}
          value={searchForm || ""}
        />
